...
//struct gPadBuffer * XInputHookControl::g_padbuffer;
//struct _XINPUT_STATE * gPadBuffer::gPadState;
...
{
	if(!XInputHookControl::UsingGamepad()) { return false; }
...
{
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(XInputHookControl::BufferedKeyPress(key))
	{
...
bool gPadBuffer::GetKeyDisabledState(UInt32 key)
{
	...
	else { return (gButtonsDisabled & this->GetMappedButton(key)) == this->GetMappedButton(key); }
}

void gPadBuffer::PostKeyEventWait() { PostKeyEventWait(POST_KEY_EVENT_WAIT()); }
void gPadBuffer::PostKeyEventWait(UInt32 interval) { Sleep(interval); }

bool gPadBuffer::IsKeyDown(UInt32 lookup_key, UInt32 key_data)
{
	if(lookup_key == 0xFFFFFFFF) { return false; }
...
bool gPadBuffer::GetKeyState(UInt32 key)
{
	...
	if(this->IsExtendedControlName(control_name))
	{
		...
		if(strcmp(control_name,Forward())==0) { if (sThumbLY > p_zone) { return true; } else { return false; } }
		else if(strcmp(control_name,Back())==0) { if (sThumbLY < -p_zone) { return true; } else { return false; } }
		else if(strcmp(control_name,StrafeLeft())==0) { if (sThumbLX < -p_zone) { return true; } else { return false; } }
		else if(strcmp(control_name,StrafeRight())==0) { if (sThumbLX > p_zone) { return true; } else { return false; } }
		else { return false; }
	}
...
bool gPadBuffer::disable(UInt32 key)
{
	...
	else { gButtonsDisabled |= this->GetMappedButton(key); }
	return true;
}
...
{
	if(!gpadconf::ExtendedControlsEnabled()) { return false; }
	/*
	...
	*/
	...
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(strcmp(control_name,Forward())==0) {if(!this->IsTextInputActive()){ b__gsThumbLY = 1; gsThumbLY = THUMB_MAX();} return true; }
	else if(strcmp(control_name,Back())==0) { if(!this->IsTextInputActive()){b__gsThumbLY = 1; gsThumbLY = THUMB_MIN();} return true; }
	else if(strcmp(control_name,StrafeLeft())==0) { if(!this->IsTextInputActive()){b__gsThumbLX = 1; gsThumbLX = THUMB_MIN();} return true; }
	else if(strcmp(control_name,StrafeRight())==0) { if(!this->IsTextInputActive()){b__gsThumbLX = 1; gsThumbLX = THUMB_MAX();} return true; }
	...
	else if(strcmp(control_name,AutoMove())==0)
	{
		if(!this->IsTextInputActive())
		{
		if(this->b__automove) { this->b__automove=0; this->pop_extended(Forward(), bHeld); }
		else { this->b__automove = 1; this->push_extended(Forward(), bHeld); }
		}
		return true;
	}
	else { return false; }
...
{
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(!this->IsGamepadKey(key)) { return false; }
	const char* control_name=this->GetControlNameFromKey(key);
	if(IsExtendedControlName(control_name)) { return this->push_extended(control_name,bHeld); }
	if(IsLeftTrigger(key))
	{
		// have LT
		gLeftTrigger = 1;
...
bool gPadBuffer::pop_extended(const char* control_name, bool bHeld)
{
	...
	*/
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(strcmp(control_name,Forward())==0) {if(!this->IsTextInputActive()){ b__gsThumbLY = 0; gsThumbLY = 0; }return true; }
	else if(strcmp(control_name,Back())==0) { if(!this->IsTextInputActive()){b__gsThumbLY = 0; gsThumbLY = 0;} return true; }
	else if(strcmp(control_name,StrafeLeft())==0) { if(!this->IsTextInputActive()){b__gsThumbLX = 0; gsThumbLX = 0;} return true; }
	else if(strcmp(control_name,StrafeRight())==0) { if(!this->IsTextInputActive()){b__gsThumbLX = 0; gsThumbLX = 0;} return true;	}
	else if(strcmp(control_name,QuickInventory())==0) { return true; }
	else if(strcmp(control_name,QuickMagic())==0) { return true; }
	else if(strcmp(control_name,QuickMap())==0) { return true; }
	else if(strcmp(control_name,QuickStats())==0) { return true; }
	else if(strcmp(control_name,Console())==0) { return true; }
	else if(strcmp(control_name,Screenshot())==0)  { return true; }
	else if(strcmp(control_name,MultiScreenshot())==0) { return true; }
	else if(strcmp(control_name,AutoMove())==0) { return true; }
	else { return false; }
}
bool gPadBuffer::pop(UInt32 key, bool bHeld)
{
	...
	if(!this->IsGamepadKey(key)) { return false; }
	const char* control_name=this->GetControlNameFromKey(key);
	if(IsExtendedControlName(control_name)) { return this->pop_extended(control_name,bHeld); }
	if(IsLeftTrigger(key))
	{
		// have LT
		gLeftTrigger = 0;
...
bool gPadBuffer::IsExtendedControlName(const char* control_name)
{
	...
	if(strcmp(control_name,Forward())==0) { return true; }
	else if(strcmp(control_name,Back())==0) { return true; }
	else if(strcmp(control_name,StrafeLeft())==0) { return true; }
	else if(strcmp(control_name,StrafeRight())==0)	{ return true; }
	else if(strcmp(control_name,QuickInventory())==0) { return true; }
	else if(strcmp(control_name,QuickMagic())==0) { return true; }
	else if(strcmp(control_name,QuickMap())==0) { return true;	}
	else if(strcmp(control_name,QuickStats())==0) { return true; }
...
const char * gPadBuffer::GetControlNameFromKey(UInt32 key)
{
	...
	if (inputManager)
	{
		UInt32 buttonID;
		UInt32 deviceType;
		if (key >= InputMap::kMacro_GamepadOffset) { buttonID = GetMappedButton(key); deviceType = kDeviceType_Gamepad; }
		else if (key >= InputMap::kMacro_MouseButtonOffset) { buttonID = key - InputMap::kMacro_MouseButtonOffset; deviceType = kDeviceType_Mouse; }
		else { buttonID = key; deviceType = kDeviceType_Keyboard; }
		if(deviceType == kDeviceType_Gamepad && buttonID == 0xFF) { buttonID = key; deviceType = kDeviceType_Keyboard; }
...
{
	if(!XInputHookControl::UsingGamepad()) { return false; } // will return false if not using a gamepad
	...
	// check if we have an exteneded or 'virual' key like Forward, Back, etc
	return this->IsExtendedControlKey(key);
}
bool gPadBuffer::IsLeftTrigger(UInt32 key)
{
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(key == InputMap::kGamepadButtonOffset_LT) { return true; }
	return false;
}
bool gPadBuffer::IsRightTrigger(UInt32 key)
{
	if(!XInputHookControl::UsingGamepad()) { return false; }
	if(key == InputMap::kGamepadButtonOffset_RT) { return true; }
	return false;
}
...
HRESULT gPadBuffer::CreatePadState(DWORD dwUserIndex, XINPUT_STATE * pState)
{
	...
	if(hr != ERROR_SUCCESS) { Log::VerboseDebug("gPadBuffer::CreatePadState::HRESULT %d", hr); return hr; }
	...
	//if(!this->IsTextInputActive())
	//{
		if(this->b__gsThumbLX) { pState->Gamepad.sThumbLX = this->gsThumbLX; }
		if(this->b__gsThumbLY) { pState->Gamepad.sThumbLY = this->gsThumbLY; }
		#if ENABLE_MOUSE_MOVEMENT
